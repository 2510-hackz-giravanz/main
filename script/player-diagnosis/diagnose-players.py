...
選手の以下の情報から念系統を判定してください：
- ニックネーム、出身地
- サッカー観（what_is_soccer）
- 背番号へのこだわり（jersey_number_commitment）
- 試合前のルーティン（pregame_ritual）
- 注目してほしいプレー（look_at_my_play）
- 憧れの選手（hero）
- 性格を一言で（personality_one_word）
- チャームポイント（charm_point）
- 座右の銘（motto）
- ファンへのメッセージ（message_to_fans）
...
def main():
    ...
    print(f"\n完了! {len(results)}/{len(players)} 人の診断が成功しました")
